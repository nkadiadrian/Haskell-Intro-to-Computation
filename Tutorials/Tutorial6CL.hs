...
myColouring :: Colouring
myColouring Perth Green = True 
...
adjacentCitiesNotSameColour paint =
  and[ or [not (paint a c), not (paint b c) ] | a <- cities, b <- cities, adj a b, c <- colours ] 
...
formalAdjacentCitiesNotSameColour = 
  And[ Or [N (Paint a c), N (Paint b c) ] | a <- cities, b <- cities, adj a b, c <- colours ]
...
latin n =
  let rows    = [0..n-1]
      ...
      noColumnHasARepeatedDigit = 
        And[ Or[ N (r, c, d), N (r', c, d) ] | c <- columns, r <- rows, r' <- rows, r < r' , d <- digits]
      noRowHasARepeatedDigit = 
        And[ Or[ N (r, c, d), N (r, c', d) ] | c <- columns, c' <- columns, c < c',  r <- rows, d <- digits]
...
prioritise = sortOn (\(Or lits) -> length lits)    
