...
isBloodOrange (Orange variety segments) | variety == "Moro" || variety == "Tarocco" || variety == "Sanguinello" = True
                                        | otherwise = False

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments xs = sum[segments | (Orange variety segments) <- xs, isBloodOrange (Orange variety segments) == True]

-- 3.
worms :: [Fruit] -> Int
worms xs = length[(Apple variety hasWorm) | (Apple variety hasWorm) <- xs, hasWorm == True]
...
equivalent wff wffA = and[(eval x wff) == (eval x wffA)  | x <- envs (atoms (wff :&: wffA))]
