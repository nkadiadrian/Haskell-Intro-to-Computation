...
delta m s symbol = [z | (x,y,z) <- trans m, x `elem` s, y == symbol]
...
ddelta m qs symbol = canonical (delta m qs symbol)

-- 6.
next :: (Ord q) => FSM q -> [[q]] -> [[q]]
next m s = canonical (s ++ [ddelta m qs symbols | symbols <- alph m, qs <- s])


-- 7.
reachable :: (Ord q) => FSM q -> [[q]] -> [[q]]
reachable m s | s /= next m s = reachable m (next m s)
              | otherwise = s

-- 8.
dfinal :: (Ord q) => FSM q -> [[q]] -> [[q]]
dfinal m s = [qs | qs <- s, intersect qs (final m) /= []]
          
-- 9.
dtrans :: (Ord q) => FSM q -> [[q]] -> [Transition [q]]
dtrans m s = [(x,y,ddelta m x y)| x <- s, y <- alph m]
...
charFSM x = ([True, False], [x], [False], [True], [(False, x, True)])
